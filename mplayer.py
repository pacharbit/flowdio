...
class MPlayer(object):
    ...
    def __init__(self, card, cache=128):
        self.card = card
        # audio stream fifo
        self.aufifo_path = '/tmp/mplayer_aufifo_' + str(os.getpid())
        self._spawn(cache)
    ...
    def _spawn(self, cache):
        ...
        # init fifo for gapless audio playback
        self.init_playback_fifo(44100, 16, 2, 'signed-integer')

        self._mplayer = subprocess.Popen(
                [self.exe_name, '-vo', 'null', '-ao', 'pcm:nowaveheader:file=' + self.aufifo_path, '-cache', '%s'%cache, '-slave', '-quiet', '-idle'],
                #[self.exe_name, '-vo', 'null', '-ao', 'alsa:device=hw=0', '-cache', '%s'%cache, '-slave', '-quiet', '-idle'],
                stdin=subprocess.PIPE, stdout=subprocess.PIPE, bufsize=1, encoding='utf8', shell=False)
        self._cache = cache
        self._readlines()
    ...
    def init_playback_fifo(self, freq, depth, channels, signed) :
        """
        TODO
        """        
        try:
            # remove fifo file
            if os.path.exists(self.aufifo_path):
                os.remove(self.aufifo_path)

            # set playback device env variable for sox play
            os.environ["AUDIODEV"] = "hw:%s,0" % self.card

            # make audio fifo
            subprocess.run(['mkfifo', self.aufifo_path])

            self.aufifo = subprocess.Popen(
                    ['play', '-t', 'raw', '-r', str(freq), '-b', str(depth), '-c', str(channels), '-e', signed, self.aufifo_path],
                    stdin=subprocess.PIPE, stdout=subprocess.PIPE, bufsize=1, shell=False)
